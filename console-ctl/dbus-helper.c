				   ...
				   char *dbus_name, char *dbus_path)
{
	...
	instances->instances[i]->dbus_name =
		malloc(SD_BUS_MAXIMUM_NAME_LENGTH + 1);
	instances->instances[i]->dbus_path =
		malloc(SD_BUS_MAXIMUM_NAME_LENGTH + 1);

	strncpy(instances->instances[i]->dbus_name, dbus_name,
		SD_BUS_MAXIMUM_NAME_LENGTH);
	strncpy(instances->instances[i]->dbus_path, dbus_path,
		SD_BUS_MAXIMUM_NAME_LENGTH);
				 ...
				 struct dbus_console_instances *instances)
{
	...
	while (sd_bus_message_enter_container(response, 'e', "sa{sas}") == 1) {
