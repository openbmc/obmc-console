				   ...
				   char *console_id)
{
	...
	instances->instances[i]->console_id =
		malloc(SD_BUS_MAXIMUM_NAME_LENGTH + 1);

	strncpy(instances->instances[i]->console_id, console_id,
		SD_BUS_MAXIMUM_NAME_LENGTH);

	instances->count += 1;
}

int get_conflicting_console_ids(char* console_id, struct dbus_console_instances *instances, bool debug)
{
	struct sd_bus *bus;
	int status;
	...
	status = sd_bus_get_property(bus, dbus_name, dbus_path,
					    control_dbus_interface, "ConflictingConsoleIds",
					    &error, &reply, "as");

	if (status < 0) {
		fprintf(stderr, "Error: Failed to get 'ConflictingConsoleIds' property: %s\n",
                        error.message);
		sd_bus_unref(bus);
		return 1;
	}
