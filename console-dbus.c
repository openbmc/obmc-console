...
#define INTF_NAME2 "xyz.openbmc_project.Console.Access"
...
static const sd_bus_vtable console_vtable2[] = {
	SD_BUS_VTABLE_START(0),
	       ...
	       struct config *config __attribute__((unused)))
{
	...
	r = sd_bus_add_object_vtable(console->bus, NULL, obj_name, INTF_NAME2,
				     console_vtable2, console);
	if (r < 0) {
		warnx("Failed to issue method call: %s", strerror(-r));
		return;
	}
