...
static ssize_t strtokcnt(const char *str, const char sep)
{
	ssize_t n = 1;
...
static int count_mux_gpios(const char *config_mux_gpios)
{
	return (int)strtokcnt(config_mux_gpios, ',');
}
					  ...
					  const char *config_gpio_names)
{
	...
	for (size_t j = 0; j < mux->n_mux_gpios; j++) {
		status = console_mux_find_gpio_by_index(server, &current, j);
		if (status != 0) {
	...
	for (size_t i = 0; i < server->mux->n_mux_gpios; i++) {
		struct console_gpio *gpio = &server->mux->mux_gpios[i];
...
int console_server_mux_init(struct console_server *server)
{
	...
	const int ngpios = count_mux_gpios(config_gpio_names);
...
int console_mux_activate(struct console *console, bool printlogs)
{
	int status = 0;
