...
"  --logsize <SIZE_KB>  Configure log size in KB (1-1024). Default is 16KB.\n"
"",
		progname);
}
...
int main(int argc, char **argv)
{
	...
	for (;;) {
		...
		case 'c':
			...
			break;
		case 's':
			logsize_kb = strtoul(optarg, NULL, 0);
			if (logsize_kb == 0 || errno == ERANGE ||
			    errno == EINVAL) {
