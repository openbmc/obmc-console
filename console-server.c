				      ...
				      struct console **active_console)
{
	...
	const char *initially_active = config_get_section_value(
		config, console->console_id, "initially-active");
	if (!initially_active) {
		return 0;
	}
			    ...
			    struct console_server_args *args)
{
	...
	if (!active_console) {
		active_console = server->consoles[0];
	}
...
static int console_server_with_args(struct console_server_args *args)
{
	...
	console_mux_activate(initial_active, false);
