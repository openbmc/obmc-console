...
ssize_t console_server_request_pollfd(struct console_server *server)
{
	...
	if (server->debug) {
		printf("console-server: registered pollfd at index %ld\n",
		       (long)index);
	}
...
static int tty_set_sysfs_attr(struct console_server *console, const char *name,
			      int value)
{
	char *path;
	FILE *fp;
 ...
 */
void tty_init_termios(struct console_server *console)
{
 ...
 */
static void tty_init_vuart_io(struct console_server *console)
{
	assert(console->tty.type == TTY_DEVICE_VUART);
...
static int tty_init_vuart(struct console_server *console,
			       ...
			       int events)
{
	...
	console->server->pollfds[i].events = (short)(events & 0x7fff);
}
...
static int run_console_iteration(struct console_server *server)
{
	...
	if (server->debug) {
		printf("console-server: calculated timeout: %ld\n", timeout);
		printf("console-server: polling on %ld file descriptors\n",
		       (long)server->n_pollfds);
	}
...
{
	if (server->debug) {
		printf("console server: entered %s\n", __func__);
	}
