...
static int tty_init_vuart(struct console_server *server)
{
	...
	const char *config_sirq = config_get_value(server->config, "sirq");
				   ...
				   const char *tty_arg)
{
	...
	} else if ((val = config_get_value(server->config, "upstream-tty"))) {
	...
	case TTY_DEVICE_UART:
		val = config_get_value(server->config, "baud");
		if (val) {
				   ...
				   struct timeval *tv)
{
	...
	if (console->rb->size < buf_size) {
		warnx("Ringbuffer size should be greater than %zuB\n",
		      buf_size);
		return 1;
	}
...
static void console_server_console_fini(struct console *console)
{
	...
	if (console->mux_index) {
		free(console->mux_index);
	}
