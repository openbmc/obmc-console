...
static void uart_routing_init(struct config *config)
{
	...
	if (glob(ASPEED_UART_ROUTING_PATTERN, GLOB_ERR | GLOB_NOSORT, NULL,
		 &globbuf) != 0) {
		warnx("Couldn't find uart-routing driver directory, cannot apply config");
		return;
	}
	if (globbuf.gl_pathc != 1) {
		warnx("Found %zd uart-routing driver directories, cannot apply config", globbuf.gl_pathc);
		goto out_free_glob;
	}
	...
	while (*p) {
		...
		if (sscanf(p, " %[^: \t]:%[^: \t] %zn", sink, source,
			   &bytes_scanned) != 2) {
			warnx("Invalid syntax in aspeed uart config: '%s' not applied", p);
			break;
		}
		p += bytes_scanned;
		...
		if ((f = fopen(path, "w")) == NULL
		    || fprintf(f, "%s", source) < 0
		    || fclose(f)) {
			warn("Failed to apply uart-routing config '%s:%s'", sink, source);
		}
	}
