			      ...
			      int old_fd)
{
	...
	if (rc) {
		warn("Failed to rename %s to %s, errno: %d\n", filename, rotate_filename, errno);
	}

	/* open new empty file */
	int new_fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (new_fd < 0) {
		warn("Can't open log file %s, errno: %d\n", filename, errno);
		return -1;
	}
	     ...
	     int *line_count_text)
{
	...
	if (*size + len >= maxsize) {
		if (log_trim(fd, filename, rotate_filename, size,
			     line_count_text) < 0) {
			warn("log_trim failed in log_data, errno: %d", errno);
			return -1;
		}
	}

	int write_rc = write_buf_to_fd(*fd, buf, len);
	if (write_rc < 0) {
		warn("write_buf_to_fd failed in log_data, rc: %d, errno: %d", write_rc, errno);
		return -1;
	}
