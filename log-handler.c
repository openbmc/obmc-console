...
#define LOGFILE_O_TRUNC  (O_WRONLY | O_CREAT | O_TRUNC)
#define LOGFILE_O_APPEND (O_WRONLY | O_CREAT | O_APPEND)
...
static int log_trim(struct log_handler *lh)
{
	...
	lh->fd = open(lh->log_filename, LOGFILE_O_TRUNC, 0644);
	if (lh->fd < 0) {
...
static int log_create(struct log_handler *lh)
{
	...
	lh->fd = open(lh->log_filename, LOGFILE_O_APPEND, 0644);
	if (lh->fd < 0) {
		warn("Can't open log buffer file %s", lh->log_filename);
		return -1;
	}
	...
	if (pos < 0) {
		close(lh->fd);
		warn("Can't lseek log buffer file %s", lh->log_filename);
		return -1;
	}
