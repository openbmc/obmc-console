...
static char *console_id_1 = "testconsole1ad";
static char *console_id_2 = "testconsole2ad";
...
static bool test_activate_deactivate_console_dbus(struct sd_bus *bus)
{
	...
	status = activate_console_by_id(bus, console_id_1, false);
	if (status != 0) {
		kill(pid, SIGINT);
	}
	assert(status == 0);
	...
	status = activate_console_by_id(bus, console_id_2, false);
	if (status != 0) {
		kill(pid, SIGINT);
	}
	assert(status == 0);
	...
	assert_contains(buf, "testconsole1ad active");
	assert_contains(buf, " DISCONNECTED");
	assert_not_contains(buf, "testconsole2ad active");
	...
	assert_contains(buf, "testconsole2ad active");
	assert_not_contains(buf, "testconsole1ad active");
