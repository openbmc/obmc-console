...
static void test_parse_binary_string(void)
{
	...
	assert(parse_binary_string("1", &out) == 0 && out == 1 && "parse_binary_string('1') should return 0 and out=1");
	assert(parse_binary_string("0", &out) == 0 && out == 0 && "parse_binary_string('0') should return 0 and out=0");

	/* Invalid inputs */
	assert(parse_binary_string("2", &out) == -1 && "parse_binary_string('2') should return -1");
	assert(parse_binary_string("-1", &out) == -1 && "parse_binary_string('-1') should return -1");
	assert(parse_binary_string("abc", &out) == -1 && "parse_binary_string('abc') should return -1");
	assert(parse_binary_string(" 1", &out) == -1 && "parse_binary_string(' 1') should return -1");
	assert(parse_binary_string("1 ", &out) == -1 && "parse_binary_string('1 ') should return -1");
	assert(parse_binary_string("", &out) == -1 && "parse_binary_string('') should return -1");
}
...
static void test_sanitize_to_ascii_ansi(void)
{
	...
	out_len = sanitize_to_ascii_ansi((uint8_t *)in1, strlen(in1), out,
					 sizeof(out), &st);
	out[out_len] = '\0';
	assert(strcmp(out, "Hello, World!\nThis is a test.") == 0 && "sanitize_to_ascii_ansi failed for in1");
	...
	out_len = sanitize_to_ascii_ansi((uint8_t *)in2, strlen(in2), out,
					 sizeof(out), &st);
	out[out_len] = '\0';
	assert(strcmp(out, "Partial") == 0 && "sanitize_to_ascii_ansi failed for in2");
	...
	out_len = sanitize_to_ascii_ansi((uint8_t *)in3, strlen(in3), out,
					 sizeof(out), &st);
	out[out_len] = '\0';
	assert(strcmp(out, " and more text.") == 0 && "sanitize_to_ascii_ansi failed for in3 continuation");
	assert(st.mode == ANSI_NORMAL && "ANSI mode should be normal after completion");
}
...
static void test_generate_timestamp_text(void)
{
	...
	assert(len > 0 && "Generated timestamp length should be greater than 0");

	/* Expected format: "Www Mmm dd hh:mm:ss yyyy 0000123 " */
	assert(strlen(buf) == 24 + 1 + 7 + 1 && "Generated timestamp length is incorrect"); /* date + space + number + space */
	assert(buf[len - 1] == ' ' && "Last character of timestamp should be space");
	assert(strstr(buf, " 0000123 ") != NULL && "Timestamp should contain line count");
}
...
static void test_log_rotation(void)
{
	...
	int fd_for_call_1 = open(log_file, O_WRONLY | O_APPEND, 0644); // Open before call
	assert(fd_for_call_1 >= 0 && "Open for first log_data call failed");
	int log_data_rc = log_data(&fd_for_call_1, &size, maxsize, log_file, rotate_file, data, sizeof(data),
			 &line_count);
	assert(log_data_rc == 0 && "First log_data call failed");
	struct stat st_temp_1;
	assert(fstat(fd_for_call_1, &st_temp_1) == 0 && "fstat failed after first log_data");
	...
	int fd_for_call_2 = open(log_file, O_WRONLY | O_APPEND, 0644); // Open before call
	assert(fd_for_call_2 >= 0 && "Open for second log_data call failed");
	log_data_rc = log_data(&fd_for_call_2, &size, maxsize, log_file, rotate_file, data, sizeof(data),
			 &line_count);
	assert(log_data_rc == 0 && "Second log_data call failed");
	struct stat st_temp_2;
	assert(fstat(fd_for_call_2, &st_temp_2) == 0 && "fstat failed after second log_data");
	size = st_temp_2.st_size;
	close(fd_for_call_2); // Close after call and fstat
	assert(size == 50 && "size after second write should be 50 (after rotation and rewrite)");

	/* 3. Third write, should trigger rotation */
	int fd_for_call_3 = open(log_file, O_WRONLY | O_APPEND, 0644); // Open before call
	assert(fd_for_call_3 >= 0 && "Open for third log_data call failed");
	log_data_rc = log_data(&fd_for_call_3, &size, maxsize, log_file, rotate_file, data, sizeof(data),
			 &line_count);
	assert(log_data_rc == 0 && "Third log_data call failed");
	struct stat st_temp_3;
	assert(fstat(fd_for_call_3, &st_temp_3) == 0 && "fstat failed after third log_data");
