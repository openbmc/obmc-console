...
static int set_terminal_baud(struct tty_handler *th, const char *tty_name,
	...
	printf("Set %s terminal baud rate to %d\n", tty_name, speed);
		...
		struct config *config __attribute__((unused)))
{
	...
	if (tty_baud != NULL) {
		if (config_parse_baud(&desired_speed, tty_baud) != 0) {
			fprintf(stderr, "%s is not a valid baud rate for \
				terminal %s\n", tty_baud, tty_name);
		} else if (set_terminal_baud(th, tty_name, desired_speed) != 0)
